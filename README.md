...
- Leaderboards backed by real player data across all 12 tabs (Playtime, Deaths, Mob Kills, Blocks Mined, Ores Mined, Distance, Advancements, XP Levels, Peaceful Rep, Outlaw Rep, Violence Rep, Lawmen) with mc-heads.net player avatars. Rep tabs fetch from the plugin's reputation context via the stats proxy; rep values display rounded to the nearest tenth. Every player row links through to a per-player advancements page (`/player/<uuid>`)
